...
def load_volatility_forecasts(conn):
    """Load GARCH forecasts and realised volatility"""
    print("\n1. Loading volatility forecasts...")
    
    # Load GARCH forecasts
    # Columns: Date, 0 (the forecast value)
    garch_df = pd.read_csv('outputs/forecasts/garch_volatility_forecast.csv')
    garch_df.columns = ['date', 'garch_forecast']  # Rename second column
    garch_df['date'] = pd.to_datetime(garch_df['date'])
    
    print(f"   GARCH data shape: {garch_df.shape}")
    print(f"   GARCH date range: {garch_df['date'].min()} to {garch_df['date'].max()}")
    
    # Load realised volatility
    # Columns: Date, adj_log_return, realised_vol_21d
    vol_df = pd.read_csv('data/processed/realised_volatility_21d.csv')
    vol_df = vol_df[['Date', 'realised_vol_21d']]  # Keep only date and volatility
    vol_df.columns = ['date', 'realised_volatility']
    vol_df['date'] = pd.to_datetime(vol_df['date'])
    
    print(f"   Realised vol data shape: {vol_df.shape}")
    print(f"   Realised vol date range: {vol_df['date'].min()} to {vol_df['date'].max()}")
    ...
    if len(merged_df) == 0:
        print("   ✗ Warning: No matching dates found between GARCH and realised volatility!")
        return
    ...
    for _, row in merged_df.iterrows():
        try:
            cursor.execute("""
                ...
                (date, garch_forecast, realised_volatility, forecast_error, abs_error)
                VALUES (%s, %s, %s, %s, %s)
